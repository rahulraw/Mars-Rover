...
#spd value given from RC from -100 to 100.
def drive(spd):
  safetyConstant = 0.5;
  
  if (spd > 0):
	M1Forward(int(safetyConstant*abs(spd)));

  elif (spd < 0):
	M1Backward(int(safetyConstant*abs(spd)));
  else:
	M1Forward(0);
	M1Backward(0);


#port = serial.Serial("/dev/ttyACM2", baudrate=38400, timeout=1)
...
#    for i in range(1, 200):
 #               M1Backward(30)
  #              time.sleep(0.01)
   ...
   #             m1EncVal = readM1encoder()
    #            print("Backward Val:%20d Status:%d" % (m1EncVal[0], m1EncVal[1]))
